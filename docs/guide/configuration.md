...
A `[HELD]` badge on a row means that package has an upgrade you can take *and* something newer that was withheld; press `i` to see which version and how old it is. A package whose *every* newer version is inside the window is no longer outdated, so it has no row at all; the picker header counts those separately as "fully held", and `c` brings them into the list. They can't be selected, because there is nothing to upgrade to yet. When they are all that is left, the picker closes once the scan finishes and the summary counts them, unless `c` has them on screen (inup remembers the toggle).
...
- `CI` — when set to anything other than `false`, `0` or an empty value, inup runs headless (report mode) instead of opening the UI
