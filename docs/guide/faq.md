...
- **`--apply`**: the only non-interactive write path — it applies the bumps and runs install. See [CI & scripting](ci.md).

By default the range prefix is preserved (`^`/`~`); pass [`--save-exact`](cli.md) to write exact versions instead.

## Why is a package (or directory) being skipped?

A few things cause inup to skip something:

- **Ignored packages** — anything matching `ignore` in [`.inuprc`](configuration.md) or `-i, --ignore` (glob supported, e.g. `@babel/*`).
...
Node `22.19` or newer (`engines.node`). `npx inup` uses your local Node; the [GitHub Action](github-action.md) defaults to Node 22.
