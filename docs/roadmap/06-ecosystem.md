...
| 2 | **Opt-in prerelease versions** (`--pre`) | 🟡 | M | 1–2d | **Partially done:** when the *installed* version is itself a prerelease, newer prereleases (same major.minor.patch tuple) and the effective latest across channels are detected by default — `parseVersions` now keeps prereleases in a separate `prereleaseVersions` list ([versions.ts](../../src/shared/versions.ts)). Remaining scope: a `--pre` flag/toggle so users on *stable* versions can opt into prerelease updates too. |
