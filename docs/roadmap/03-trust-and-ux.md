...
|---|---|:--:|:--:|---|---|
| 1 ✅ | **Extract the keymap into one declarative table** | 🔴 | M | 1–2d | `KEY_BINDINGS` in [keymap.ts](../../src/ui/keymap.ts) is now the single source of truth. Drives dispatch, the `?` overlay, the footer hints, and the README table. |
| 2 ✅ | **`?` in-app help overlay** — rendered from the keymap table | 🔴 | S | 0.5d | [help-modal.ts](../../src/ui/renderer/help-modal.ts) renders directly from `getHelpGroups()`. Scrollable, always correct. |
