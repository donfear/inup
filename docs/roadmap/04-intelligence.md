...
> (`accept: application/vnd.npm.install-v1+json`, [npm-registry.ts:88](../../src/shared/registry/npm-registry.ts#L88)),
...
|---|---|:--:|:--:|---|---|
| 1 | **Vuln-fix verdict — does the upgrade _clear_ the advisory?** | 🔴 | M | 2–3d | **The marquee feature.** [vulnerability-checker.ts](../../src/features/audit/vulnerability-checker.ts) sends only the *current* version ([:62-65](../../src/features/audit/vulnerability-checker.ts#L62-L65)) and captures each advisory's `vulnerable_versions` string ([:139](../../src/features/audit/vulnerability-checker.ts#L139)) **but never compares it to the target.** Add `semver.satisfies(target, vulnerable_versions)` to mark **"✓ fixed by upgrade"** vs "still vulnerable." Turns the audit from information into guidance — and it's the highest-weight Risk Score input. *(Pairs with the only-vulnerable filter, [03](03-trust-and-ux.md) #7.)* |
...
| 9 | **Persist vulnerability results** (short-TTL cache) | 🟡 | S | 0.5d | Advisories refetch every session; reuse [persistent-cache.ts](../../src/shared/registry/persistent-cache.ts) with a short TTL. Keeps the security signal off the critical path (a north-star non-negotiable: *fast*). |
