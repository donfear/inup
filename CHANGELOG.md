...
- Packages whose every newer version is inside the cooldown window are not outdated, so they have no row in the picker. The header counts those separately as "fully held" and `c` brings them into the list, showing which version is being held. They can't be selected — there is nothing to upgrade to yet. A `[HELD]` badge on a normal row means something different: that package has an upgrade you can take, and something newer than it was withheld.
