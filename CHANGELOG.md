...
- `INUP_FASTLINK=0` keeps the adaptive controller on but never lets throughput pin the ceiling (A/B toggle). `INUP_PACE_BPS=<bytes/s>` paces every streamed registry chunk so slow-link behaviour can be reproduced without a system link conditioner; both are captured in `INUP_PERF=1` records.
