...
describe('version-checker', () => {
  describe('checkForUpdate()', () => {
    it(`should check for updates for ${TEST_PACKAGE_NAME}`, async () => {
      const result = await checkForUpdate(TEST_PACKAGE_NAME, '1.0.0')
    ...
    it('should detect up-to-date version', async () => {
      ...
      const result = await checkForUpdate(TEST_PACKAGE_NAME, '999.999.999')
    ...
    it('should have update command', async () => {
      ...
      expect(result?.updateCommand).toContain(TEST_PACKAGE_NAME)
    }, 10000)
  })

  describe('checkForUpdateAsync()', () => {
    it(`should check for updates asynchronously for ${TEST_PACKAGE_NAME}`, async () => {
      const result = await checkForUpdateAsync(TEST_PACKAGE_NAME, '1.0.0')
