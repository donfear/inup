...
- **Default skip list** — inup does not descend into `node_modules`, `dist`, `build`, `coverage`, `out`, `lib`, `es`, `esm`, `cjs`, `__fixtures__`, or `__mocks__`. If a real package lives under one of those (e.g. `lib/`), add it to `scanDirs` in `.inuprc`.
